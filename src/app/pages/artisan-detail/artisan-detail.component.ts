...
export class ArtisanDetailComponent implements OnInit {
  ...
  onInterested(service: ServiceDetail) {
    ...
    if (!this.authService.isLoggedIn()) {
      ...
      this.router.navigate(["/login"], {
        queryParams: {
          returnUrl: `/service-confirmation`,
    ...
    if (!currentUser?.roles.includes("CLIENT")) {
      // Logged in but not a client - show message or redirect
      alert(
        "Only clients can request services. Please log in with a client account."
      );
      return;
    }

    // User is authenticated and is a client - proceed to confirmation
  ...
  //   // User is authenticated - proceed to confirmation page
  //   this.router.navigate(["/service-confirmation"], {
  //     queryParams: {
  //       serviceId: service.id,
  //       artisanId: this.artisan?.id,
